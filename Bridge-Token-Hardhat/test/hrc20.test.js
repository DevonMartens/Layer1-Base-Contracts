...
describe("Testing the initial values to validate expected contract state", function () {
  ...
  it("Backed HRC20: have correct values for name & symbol", async () => {
  ...
  it("Upon deployment tokens minted should equal totalSupply", async () => {
    expect(await BackedHRC20Contract.totalSupply()).to.equal(900);
  });
  it("initalize should only be called upon deployment", async () => {
    await expectRevert(
      BackedHRC20Contract.initialize("HAVEN1", "HRC20", ContractDeployer, ContractDeployer),
      "Initializable: contract is already initialized"
    );
  });

  it("Backed HRC20: the deposit function should mint the correct amount of tokens to the designated wallet", async () => {
  ...
  it("Backed HRC20: the withdraw function should revert and give the error INSUFFICENT_BALANCE if a request is made to withdraw more than the balance", async () => {
    await expectRevert(BackedHRC20Contract.redeemBackedToken(ContractDeployer), `110`);
  });
  it("Backed HRC20: function pause function should stop all deposits", async () => {
    await BackedHRC20Contract.pause();
    await expectRevert(BackedHRC20Contract.issueBackedToken(Address2, 900), "Pausable: paused");
  });
  it("Backed HRC20: function pause function should stop all users from calling redeemBackedToken", async () => {
    ...
    await BackedHRC20Contract.pause();
    await expectRevert(Address2SignsBackedHRC20.redeemBackedToken(450), "Pausable: paused");
  });
  it("Backed HRC20: function pause function should stop all transfers", async () => {
    await BackedHRC20Contract.pause();
    await expectRevert(BackedHRC20Contract.transfer(ContractDeployer, 450), "Pausable: paused");
  });
  ...
  it("Backed HRC20: only the operator role should be able to call burnFrom ", async () => {
    ...
    await expectRevert(
      Address2SignsBackedHRC20.burnFrom(ContractDeployer, 70, "I_WANT_TO_BURN_IT_ALL_DOWN!"),
      `AccessControl: account ${Address2ErrorMessageForAccessControl} is missing role ${OPERATOR_ROLE}`
    );
  });
  ...
  it("upgrades should only be allowed to be called by DEFAULT_ADMIN_ROLE", async function () {
    ...
    await expectRevert(
      upgrades.upgradeProxy(HRC20HasADifferentUpgrader.address, BackedHRC20Factory, {
        kind: "uups",
      }),
      `AccessControl: account ${ContractDeployerErrorMessageForAccessControl} is missing role ${DEFAULT_ADMIN_ROLE}`
    );
    const HRC20V2 = await upgrades.upgradeProxy(BackedHRC20Contract.address, BackedHRC20Factory, {
      kind: "uups",
    });
  });
  ...
  it("Backed HRC20: the withdraw function should revert and give the error BALANCE_TOO_LOW if a request is made to withdraw more than the balance", async () => {
    await expectRevert(Address2SignsBackedHRC20.redeemBackedToken(Address2), `110`);
  });
  ...
  it("Backed HRC20: A wallet that is not a contract should not be allowed to be approved by the function increaseAllowance", async () => {
  ...
  it("Backed HRC20: A wallet address that is not a contract NOT be allowed to be approved by ERC20 function approve", async () => {
  ...
  it("Backed HRC20: A contract should not be allowed to be approved by the function increaseAllowance", async () => {
    ...
    await BackedHRC20Contract.increaseAllowance(BackedHRC20ContractAlternateAdminAndOperators.address, 8);
    expect(await BackedHRC20Contract.allowance(ContractDeployer, BackedHRC20ContractAlternateAdminAndOperators.address)).to.equal(
      8
    );
  });
  it("Backed HRC20: A contract should not be allowed to be approved by the function approve", async () => {
    ...
    await BackedHRC20Contract.approve(BackedHRC20ContractAlternateAdminAndOperators.address, 8);
    expect(await BackedHRC20Contract.allowance(ContractDeployer, BackedHRC20ContractAlternateAdminAndOperators.address)).to.equal(
      8
    );
  });
