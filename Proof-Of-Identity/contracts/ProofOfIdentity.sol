    ...
    UUPSUpgradeable
{
    ...
    */
    ...
    /** 
    * @dev The event is triggered when `issueIdentity` is called. 
    ...
    */
    ...
    /**
     * @dev The event is triggered during the `establishCompetencyRating` function.
     * It sends the address that has been give a competency rating and the 
     * score it was granted.
     */
    event CompetencyRatingEstablished(
        address indexed account,
    ...
    // Error that occurs when an address tries to mint a token but has already been verified.
    error PreviouslyVerified();

    // Error that occurs when a function tries to call a token that does not exist.
    error IDDoesNotExist();

    // Error that occurs when an address tries to transfer a token.
    error IDNotTransferable();
    ...
    function establishCompetencyRating(
        ...
        uint8 score
            ...
            emit CompetencyRatingEstablished(account, score);
    }
