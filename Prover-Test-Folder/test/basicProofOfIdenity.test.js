...
describe("Testing the initial values to validate expected contract state", function () {
    ...
    describe("Testing the mintIdentity to validate expected contract state", function () {
        ...
        beforeEach(async() => {
            ...
            ProofOfIdentityContract = await upgrades.deployProxy(ProofOfIdentity, [IPermissionsInterfaceDummyInstance.address, owner, owner], { initializer: 'initialize' });
    ...
    describe("Testing contracts that inhert OtherInformation and RoleVerification should view correct values for a set struct of identityBlob", function () {      
        ...
        it("The values for `user type` in a seperate verifiable identity contract should match the values for the original  proof of identity", async () => {
            ...
            //checks against each other
            expect(await VerifiableIdentity.getUserType(alice)).to.equal(await ProofOfIdentityContract.getUserAccountType(alice))
            expect(await VerifiableIdentity.getUserType(other)).to.equal(await ProofOfIdentityContract.getUserAccountType(other))
        });
        it("The values for `level` in a seperate verifiable identity contract should match the values for the original  proof of identity", async () => {
            ...
            expect(await ProofOfIdentityContract.getUserAccountLevel(alice)).to.equal(3)
            expect(await ProofOfIdentityContract.getUserAccountLevel(other)).to.equal(6)
            ...
            //checks against each other
            expect(await VerifiableIdentity.getUserLevel(alice)).to.equal(await ProofOfIdentityContract.getUserAccountLevel(alice))
            expect(await VerifiableIdentity.getUserLevel(other)).to.equal(await ProofOfIdentityContract.getUserAccountLevel(other))
        });
        it("The contract: values for `expiry` in the test caller contract should match the values for the original contract", async () => {
            ...
            expect(await await VerifiableIdentity.getUserExpiry(other)).to.equal(78886932789)
            //checks against each other
            expect(await VerifiableIdentity.getUserExpiry(alice)).to.equal(await ProofOfIdentityContract.getUserAccountExpiry(alice))
            expect(await await VerifiableIdentity.getUserExpiry(other)).to.equal(await ProofOfIdentityContract.getUserAccountExpiry(other))
        });
